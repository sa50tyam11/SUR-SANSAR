...
export default function RootLayout({
  ...
  return (
    <html lang="en" className={`dark ${inter.variable} ${playfair.variable} ${greatVibes.variable}`}>
      <body className="bg-background-dark text-slate-200 font-sans min-h-screen selection:bg-gold-500/30">
        <header className="absolute top-0 w-full z-50 px-6 md:px-12 lg:px-24 py-8 flex items-center justify-between pointer-events-none">
          ...
          <div className="hidden md:flex flex-col items-center pointer-events-auto mr-4">
            <div className="flex items-center gap-4 font-display text-[#D6A95B] text-sm tracking-[0.2em] uppercase font-medium">
              <button className="hover:text-white transition-colors duration-300">DISCOVER</button>
              <span className="text-[#D6A95B]/60">•</span>
              <button className="hover:text-white transition-colors duration-300">LISTEN</button>
              <span className="text-[#D6A95B]/60">•</span>
              <button className="hover:text-white transition-colors duration-300">CONNECT</button>
            </div>
